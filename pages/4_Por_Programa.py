...
Vista detallada por programa académico
"""
...
def _master_activities_table_html(df: pd.DataFrame) -> str:
    ...
    cur_per = None
    row_idx = 0
    sort_cols = [c for c in ("NOMBRE DEL PROGRAMA", "PERIODO DE IMPLEMENTACIÓN") if c in df.columns]
...
st.markdown(
    ...
    '<div style="font-size:12px;color:rgba(255,255,255,0.70);margin-top:5px">Fase 2 · Por Programa</div></div>',
    unsafe_allow_html=True,
)
